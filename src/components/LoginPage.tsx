...
const LoginPage = () => {
    ...
    const [errorText, setErrorText] = useState(
        state == AuthState.Failed ? "authFailed" : ""
    );
    ...
    const loginMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data.success) validateSession();
            else setErrorText(data.errorMessage ? data.errorMessage : "");
        },
        onError: () => {
    ...
    useEffect(() => {
        if (state == AuthState.Failed) setErrorText("authFailed");
    }, [state]);
    ...
    return (
        <div className="login-page">
            <div className="login-form">
                ...
                <Button onPress={() => loginMutation.mutate()} isDisabled={buttonDisabled() || state == AuthState.Pending}>
                    {state == AuthState.Pending ? <SmallLoader/> : t("submit")}
                </Button>
                {errorText == "authFailed" ? (
                    <div className="login-error-message">{t("authFailed")}</div>
                ) : (
                    <></>
                )}
                <div className="login-no-account-text">
                    {t("noAccount")} <Link to={"/register"}>{t("here")}</Link>
                </div>
            </div>
        </div>
    );
